...
- **Full posting, not a summary.** The single most important extraction rule: `description`, `requirements`, and `responsibilities` capture the complete text of the original posting, cleaned of navigation and footer cruft but never shortened, paraphrased, or truncated to "the important parts." If a posting has five paragraphs of requirements, the stored field has five paragraphs. `source_url` (and `external_url` when it differs) always points back to the original page, so a user can verify against the source directly regardless of extraction quality; the opportunity detail page links to it prominently for exactly this reason.
- **Source config, not hardcoded scrapers per site.** Each source is a small config (base URL, list page selector or API, detail page selectors, `needs_js` flag) so adding a new organization is a config entry, not new code, wherever the site's HTML is regular enough for that.
- **Change detection first.** Before parsing, compare a hash of the fetched page against the last seen hash, stored per `source_url`, and skip parsing entirely on no change. This is a direct evolution of the dead link checker already built for the ICT directory; the same "did this actually change" logic just gets a hash check and a parse step added.
...
- **Scheduling:** GitHub Actions `schedule: - cron: "17 4 * * *"` (04:17 UTC, which is 07:17 EAT; a few minutes off the hour to avoid GitHub's busiest scheduling window, close enough to "7am" for the brief's intent). Add a `workflow_dispatch:` trigger too, for running it manually while testing. Because GitHub disables schedules after 60 days of total repo inactivity, add a lightweight scheduled heartbeat step that fails loudly (for example, posting to a free UptimeRobot or a Slack or Discord webhook) if the scrape run itself does not complete; GitHub does not notify you of scheduler failures on its own.
